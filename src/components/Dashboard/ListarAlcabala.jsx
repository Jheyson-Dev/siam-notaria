...
import { listAlcabala, getAlcabalaReportUrl } from '../../services/alcabalaService';
import { createPortal } from 'react-dom';
import PdfViewerModal from './PdfViewerModal';

const ListarAlcabala = ({ isOpen, onClose, data, connectionData }) => {
    const [alcabalaList, setAlcabalaList] = useState([]);
    ...
    const getValue = (obj, key) => {
        if (!obj) return '-';
        // Si el objeto tiene la clave directamente, retornarla
    ...
    const totals = React.useMemo(() => {
        if (!Array.isArray(alcabalaList)) return { deuda: 0, pago: 0, saldo: 0 };
        return alcabalaList.reduce((acc, item) => {
        ...
        }, { deuda: 0, pago: 0, saldo: 0 });
    }, [alcabalaList]);

    const handlePrintRow = (item) => {
        const ids = {
            ide_alc: getValue(item, 'ide_alc'),
            ide_cnt: connectionData.ide_cnt,
            ide_eje: getValue(item, 'ano_eje'),
            ide_bde: connectionData.ide_bde
        };

        alert(`Depuración de IDs:\nide_alc: ${ids.ide_alc}\nide_cnt: ${ids.ide_cnt}\nide_eje: ${ids.ide_eje}\nide_bde: ${ids.ide_bde}`);

        const url = getAlcabalaReportUrl(item, connectionData.ide_bde);
        setPdfUrl(url);
        setIsPdfOpen(true);
    };

    const fetchAlcabala = async () => {
        setIsLoading(true);
        setError('');
        try {
            ...
            const result = await listAlcabala(payload);
            console.log("Alcabala raw data:", result); // Depuración para el desarrollador
            setAlcabalaList(Array.isArray(result) ? result : []);
        } catch (err) {
            console.error("Error fetching Alcabala:", err);
            setError(err.message || 'Error al cargar los registros de Alcabala');
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="relative w-full max-w-7xl bg-white rounded-[2.5rem] shadow-2xl flex flex-col max-h-[92vh] border border-slate-100 animate-fade-scale">
                ...
                <div className="px-8 py-6 bg-slate-50 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-secondary tracking-tight">Impuesto de Alcabala</h2>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    ...
                                    <th className="px-4 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]" rowSpan="2">Alc. Nº</th>
                                    ...
                                    <th className="px-4 py-4 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]" rowSpan="2">% Cond.</th>
                                    <th className="px-4 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]" rowSpan="2">Predio Materia de Venta</th>
                                    <th className="bg-emerald-50/50 px-4 py-2 text-center text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] border-l border-emerald-100/50" colSpan="2">Datos del Vendedor</th>
                                ...
                                <tr className="bg-slate-50/50 border-b border-slate-100">
                                    <th className="px-4 py-3 text-left text-[9px] font-bold text-slate-500 uppercase tracking-widest border-l border-emerald-100/50">DNI/RUC</th>
                                    <th className="px-4 py-3 text-left text-[9px] font-bold text-slate-500 uppercase tracking-widest">Apellidos y Nombres</th>
                                    ...
                                    <th className="px-4 py-3 text-right text-[9px] font-bold text-emerald-600 uppercase tracking-widest border-l border-slate-100">Pago</th>
                                    ...
                                    <th className="px-4 py-3 text-center text-[9px] font-bold text-slate-500 uppercase tracking-widest border-l border-slate-100">Acción</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-200">
                                ...
                                ) : alcabalaList.length > 0 ? (
                                    alcabalaList.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                                            <td className="px-4 py-4 text-xs font-bold text-secondary">{getValue(item, 'ano_eje')}</td>
                                            <td className="px-4 py-4 text-xs font-bold text-slate-600">{getValue(item, 'nro_alc')}</td>
                                            <td className="px-4 py-4 text-xs font-bold text-slate-500">{formatDate(getValue(item, 'fch_alc'))}</td>
                                            ...
                                            <td className="px-4 py-4 text-center text-xs font-bold text-slate-600 border-l border-slate-200">{getValue(item, 'por_cnd')}%</td>
                                            <td className="px-4 py-4 text-[10px] font-bold text-slate-500 min-w-[300px] border-l border-slate-200" title={getValue(item, 'pre_vta')}>
                                                {getValue(item, 'pre_vta')}
                                            </td>
                                            <td className="px-4 py-4 text-xs font-mono text-slate-700 font-bold border-l border-slate-300">{getValue(item, 'doc_cnt_ven')}</td>
                                            <td className="px-4 py-4 text-[10px] font-black text-secondary leading-tight max-w-[180px] border-l border-slate-100">{getValue(item, 'nom_cnt_ven')}</td>
                                    ...
                                    <tr>
                                        <td colSpan="11" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center opacity-40">
                                                ...
                                                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No se encontraron registros de Alcabala</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                            {alcabalaList.length > 0 && !isLoading && (
                            ...
                            <div key={idx} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                                ...
                                <div className="px-5 py-4 bg-slate-50/80 border-b border-slate-100 flex justify-between items-center">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-100">
                                                Alc. Nº {getValue(item, 'nro_alc')}
                                            </span>
                                        </div>
                                        <span className="text-[10px] font-bold text-slate-400 font-mono tracking-widest uppercase italic">
                                            {formatDate(getValue(item, 'fch_alc'))}
                                        </span>
                                    </div>
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest bg-white px-2 py-1 rounded-lg border border-slate-200">
                                            {getValue(item, 'por_cnd')}% CND.
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="p-5 space-y-5">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <div className="pl-3 border-l-2 border-slate-100 ml-0.5">
                                            <p className="text-xs font-black text-secondary leading-snug">{getValue(item, 'nom_cnt_ven')}</p>
                                            <p className="text-[10px] font-bold text-slate-400 mt-0.5 font-mono tracking-wider">{getValue(item, 'doc_cnt_ven')}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                            ...
                                            Predio Materia de Venta
                                        </span>
                                        <div className="pl-3 border-l-2 border-slate-100 ml-0.5">
                                            <p className="text-[11px] font-bold text-slate-600 leading-relaxed italic">{getValue(item, 'pre_vta')}</p>
                                        </div>
                                    </div>

                                    {/* Financial Grid - Structured */}
                                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col gap-3">
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            ...
                                            <div className="flex flex-col items-center p-2 bg-rose-50/50 rounded-xl border border-rose-100 shadow-sm">
